use std::{fs::File, hash::{DefaultHasher, Hash, Hasher}, io::{self, Read}, os::{fd::AsRawFd, unix::fs::FileExt}};

use log::info;
use serde::Serialize;

use crate::{fiemap::{fs_ioc_fiemap, ioctl, FiemapExtentFlag, FiemapFlag, FiemapRequestFull}, report::{ExtentSource, ReportExtent, ReportSummary}, utils::default_vec, ResultType};

pub(crate) fn do_scan(file: &mut std::fs::File, device: &mut std::fs::File, out: &mut impl io::Write) -> ResultType<()> {
    ...
    ReportSummary{ device_length: file_length }.serialize(&mut serializer)?;
    ...
    while file_offset < file_length {
        ...
        for e in &fr.fm_extents[..fr.request.fm_mapped_extents.try_into().unwrap()] {
            ...
            if flags.contains(FiemapExtentFlag::UNWRITTEN) {
                ...
                let csum = check_equality_and_compute_checksum(file, e.fe_logical, device, e.fe_physical, readable_length)?;

                let re = ReportExtent {
                    destination_offset: e.fe_logical,
                    length: readable_length,
                    source: ExtentSource::Offset{ offset: e.fe_physical, checksum: csum }
                };
                re.serialize(&mut serializer)?;
            }
...
fn check_equality_and_compute_checksum(a: &mut File, a_offset: u64, b: &mut File, b_offset: u64, length: u64) -> ResultType<u64> {
